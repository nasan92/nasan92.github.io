---
title: "OAuth 2.0 - Azure Server Side App Auth Code Flow - Entra ID"
...
In addition to the first showcase where we simply used a localhost Web App as a "server side app" as confidential client using the Authorization Code Flow, we publish in this tutorial the app to Azure to make it a real "server side app" where no user could have access to secrets. 
...
### requirements.txt 
Within your app folder e.g. flask_app create first a requirements.txt which contains all requirements which will be used from the app within the docker image: 
...
### Dockerfile
Then create a new Dockerfile with the following content:
...
### Python Source Code
The actual code for the Python Flask sample Web App is largely the same like in the last tutorial: 
...
### Build Docker Image
NOTE: on MacOs its necessary to build the docker image explicitely for "linux/amd64":
...
Okay and now tag and push your image to your Azure Container Registry:
````bash
...
Okay and now we need to create an Azure Web App where this image can be used:

![Create-azure-web-app](/images/Azure-create-web-app.png "Preview")

And use the Image which we created before: 

![Create-azure-web-app-container](/images/Azure-Create-Web-App-container.png "Preview")


## Configure Environment Variables 
We use environment variables to get client infos like client id and secret. 
Add them with the correct names to your app:
...
Until now your App Registration had only a Redirect URI to your localhost address. Because our app now runs on an Azure Web App Service we need to change that: 

find your new URL on your Azure Web App in the Overview Section: Default Domain (For the case of simplicity we use the default domain).
Copy this domain and switch to your App Registration which we created in the last tutorial. 
In there add replace localhost with your new URI and also add the /callback to it (Used in the Python code)
...
This time it doesn't aks for consent again, because we did already consent for the same app in the last tutorial.
